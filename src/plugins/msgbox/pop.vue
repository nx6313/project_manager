  ...
  methods: {
    ...
    addPeople (nodeIndex, exeIndex) {
      this.$dialog_confirm({
        ...
        callback: (data) => {
          for (let p = 0; p < data['data-0'].split('|').length; p++) {
            this.modeNodes[nodeIndex].executors[exeIndex].peoples.splice(this.modeNodes[nodeIndex].executors[exeIndex].peoples.length, 1, data['data-0'].split('|')[p])
          }
        }
      })
    },
